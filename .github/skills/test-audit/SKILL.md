...
Always close the audit with two ready-to-use blocks:

#### GitHub Issue

Produce a GitHub Issue body in this format:

```
**File:** `<filename>`

**Summary:** <one sentence from the overall verdict>

**Missing tests (priority order):**
- [ ] <unit or e2e> — <route or method> — <scenario> → expected <status>
- [ ] ...
...
Produce a single prompt the user can paste directly into Agent mode to implement the missing tests:

```
Implement the missing tests for `<filename>` based on the following audit findings:
- <item 1 from Jetzt-list>
- <item 2 from Jetzt-list>

Follow the existing test patterns in the project. Use Supertest for E2E tests and the existing
createController/getHookFunction patterns for unit tests. Do not change any production code.
```
...
- Do not flag missing tests for behavior that is already covered by hook-level specs (e.g. jwt-required.hook.spec.ts)
...
- Always produce both the GitHub Issue block and the Follow-up Prompt at the end — never skip them
