...
const envSchema = z.object({
  ...
  DATABASE_TYPE: z.string().min(1, 'DATABASE_TYPE darf nicht leer sein'),

  DATABASE_HOST: z.string().min(1, 'DATABASE_HOST darf nicht leer sein'),
  ...
  DATABASE_USERNAME: z.string().min(1, 'DATABASE_USERNAME darf nicht leer sein'),

  DATABASE_PASSWORD: z.string().min(1, 'DATABASE_PASSWORD darf nicht leer sein'),
  ...
  DATABASE_POOL_MIN: z.string().regex(/^\d+$/).transform(Number).optional(),

  DATABASE_POOL_IDLE_TIMEOUT: z.string().regex(/^\d+$/).transform(Number).optional(),

  DATABASE_POOL_ACQUIRE_TIMEOUT: z.string().regex(/^\d+$/).transform(Number).optional(),
  ...
  DATABASE_CACHE_ENABLED: z
    .string()
    .transform(val => val === 'true')
    .optional(),

  DATABASE_CACHE_DURATION: z.string().regex(/^\d+$/).transform(Number).optional(),
});
